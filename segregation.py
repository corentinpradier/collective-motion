...
class Murmuration:
    ...
    #         sx_prey = neighbour_mask_prey.astype(float) @ cos_angle_prey
    #         sy_prey = neighbour_mask_prey.astype(float) @ sin_angle_prey

    #         avg_neighbour_angle_prey = np.arctan2(sy_prey, sx_prey)

    #         angle_new[self.is_prey] = self.couplage * avg_neighbour_angle_prey + (
    #             1 - self.couplage
    #         ) * (angle_prey + noise[self.is_prey])

    #     if np.sum(self.is_predator) > 0:
    #         # angle_new[self.is_predator] = (
    #         #     angle[self.is_predator] + noise[self.is_predator]
    #         # )

    #         # Les prédateurs cont là où il y a le plus de proies
    #         for i, predator_idx in enumerate(np.where(self.is_predator)[0]):
    #             predator_pos = position[:, predator_idx]
    #             vectors_to_preys = position_prey - predator_pos[:, None]
    #             distances = np.linalg.norm(vectors_to_preys, axis=0)
    #             if np.any(distances < self.radius_predators):
    #                 closest_prey = np.argmin(distances)
    #                 vec = vectors_to_preys[:, closest_prey]
    #                 angle_new[predator_idx] = np.arctan2(vec[1], vec[0]) + noise[predator_idx]
    #             else:
    ...
    #         # Masque des prédateurs proches
    #         predator_mask = dist_sq_pred < self.radius_predators**2

    #         fleeing_sx = np.zeros(self.n_preys)
    #         fleeing_sy = np.zeros(self.n_preys)

    #         for prey_idx in range(self.n_preys):
    #             nearby_predators = predator_mask[prey_idx]
    #             if np.any(nearby_predators):
    #                 vectors_away = diffs_pred[:, prey_idx, nearby_predators]
    #                 mean_away = np.mean(vectors_away, axis=1)
    #                 norm = np.linalg.norm(mean_away)
    #                 if norm > 0:
    #                     fleeing_sx[prey_idx] = mean_away[0] / norm
    #                     fleeing_sy[prey_idx] = mean_away[1] / norm
    #                 else:
    #                     fleeing_sx[prey_idx] = np.random.uniform(-1, 1)
    #                     fleeing_sy[prey_idx] = np.random.uniform(-1, 1)

    #         fleeing_angle = np.arctan2(fleeing_sy, fleeing_sx)

    #         fear_weight = 0.7
    #         align_weight = 1.0 - fear_weight

    #         angle_combined = np.arctan2(
    #             align_weight * np.sin(avg_neighbour_angle_prey) + fear_weight * np.sin(fleeing_angle),
    #             align_weight * np.cos(avg_neighbour_angle_prey) + fear_weight * np.cos(fleeing_angle),
    #         )

    #         angle_new[self.is_prey] = self.couplage * angle_combined + (1 - self.couplage) * (angle_prey + noise[self.is_prey])

    #     else:
    #         angle_new[self.is_prey] = self.couplage * avg_neighbour_angle_prey + (1 - self.couplage) * (angle_prey + noise[self.is_prey])
    ...
    def theta_update(self, position, angle):
        ...
        if self.n_preys > 0:
            ...
            cos_angle_prey = np.cos(angle_prey)
            sin_angle_prey = np.sin(angle_prey)
            sx_prey = neighbour_mask_prey @ cos_angle_prey
            sy_prey = neighbour_mask_prey @ sin_angle_prey

            avg_neighbour_angle_prey = np.arctan2(sy_prey, sx_prey)

            # Fuite des prédateurs
            if np.any(self.is_predator) > 0:
                diffs_pred = position_prey[:, :, None] - position_predator[:, None, :]
                dist_sq_pred = np.sum(diffs_pred**2, axis=0)
                predator_mask = dist_sq_pred < self.radius_predators**2

                fleeing_angle = np.copy(avg_neighbour_angle_prey)
                fear_weight = np.zeros(self.n_preys)

                for prey_idx in range(self.n_preys):
                    nearby_predators = predator_mask[
                        prey_idx
                    ]  # booléen sur les prédateurs
                    if np.any(nearby_predators):
                        vectors_away = diffs_pred[:, prey_idx, nearby_predators]
                        mean_away = np.mean(vectors_away, axis=1)
                        norm = np.linalg.norm(mean_away)
                        if norm > 0:
                            # On inverse complètement l'angle de fuite
                            fleeing_angle[prey_idx] = np.arctan2(
                                -mean_away[1], -mean_away[0]
                            )
                        else:
                            fleeing_angle[prey_idx] = np.random.uniform(-np.pi, np.pi)

                        # Calcul de la distance au prédateur et ajustement du poids de peur
                        mean_dist = np.mean(
                ...
                # Fuite complète avec alignement
                align_weight = 1.0 - fear_weight

                # Plus de poids sur la fuite des prédateurs
                combined_angle = np.arctan2(
                    fear_weight * np.sin(fleeing_angle)
                    + align_weight * np.sin(avg_neighbour_angle_prey),
                    fear_weight * np.cos(fleeing_angle)
                    + align_weight * np.cos(avg_neighbour_angle_prey),
                )

                # Application du changement d'angle avec un peu de bruit
                angle_new[self.is_prey] = self.couplage * combined_angle + (
                    1 - self.couplage
                ) * (angle_prey + noise[self.is_prey])
        ...
        if np.any(self.is_predator):
            ...
            for i, predator_idx in enumerate(predator_indices):
                ...
                if np.any(distances < self.radius_predators):
                    ...
                    angle_new[predator_idx] = direction + noise[predator_idx]
                else:
                    angle_new[predator_idx] = angle[predator_idx] + noise[predator_idx]
    ...
    def dont_touch_predator(self, position):
        ...
        if self.n_predators == 0:
            ...
            if indices_i.size > 0:
                ...
                correction_vectors = relevant_diffs / relevant_dist[None, :] * correction_magnitude[None, :]
